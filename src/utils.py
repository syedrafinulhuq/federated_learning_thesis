...
def remove_stop_words(text):
    stop_words = set(['the', 'a', 'of', 'and', 'to', 'in', 'is', 'that', 'on', 'for'])
    clean_text = ' '.join([word for word in text.split() if word not in stop_words])
    return clean_text

def remove_extensions(text):
    # Remove common domain and file extensions
    clean_text = re.sub(r'\.(com|org|net|html?|php)(/)?', '', text, flags=re.IGNORECASE)
    return clean_text
...
def preprocess_input(args):
    ...
    processed_output = np.array(
        [
            ...
            # list(raw_text)   # only if no reversal during training
            if i in vocabulary
        ],
        dtype=np.float32,
    )
